...
object DateUtil {
    ...
    fun getMonthformat(date: String): String {
        val d = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH).parse(date) // 2017-12-06 10:30:00
        val cal = Calendar.getInstance()

        cal.time = d
        return SimpleDateFormat("E, MMM dd, h:mm a", Locale.ENGLISH).format(cal.time) // Dec,06 10:30 AM
    }
    ...
    fun getupdateformat(date: String): String {
        ...
        cal.time = d
        return SimpleDateFormat("MMM dd, h:mm a", Locale.ENGLISH).format(cal.time) // Dec,06 10:30 AM
    }
    ...
    fun showDateDialog(context: Context, datePicker: DatePickerDialog.OnDateSetListener, hidePast: Boolean) {
        val calendar = Calendar.getInstance()
        val year = calendar.get(Calendar.YEAR)
        val month = calendar.get(Calendar.MONTH)
        val dayOfMonth = calendar.get(Calendar.DAY_OF_MONTH)

        val dialog = DatePickerDialog(context, R.style.MyDatePicker, datePicker, year, month, dayOfMonth)
    ...
    fun showFromDateDialog(
        ...
        val dialog = DatePickerDialog(context, R.style.MyDatePicker, from_datePicker, year, month, dayOfMonth)
        dialog.setCancelable(false)
    ...
    fun showToDateDialog(
        ...
        val month = calendar.get(Calendar.MONTH)
        val dayOfMonth = calendar.get(Calendar.DAY_OF_MONTH)
        val Selected = SelectedDate.split("-".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        try {
            val dialog = DatePickerDialog(context, R.style.MyDatePicker, to_datePicker, year, month, dayOfMonth)
            dialog.setCancelable(false)
    ...
    fun showTimeDialog(context: Context, listener: TimePickerDialog.OnTimeSetListener) {
        ...
        val dialog = TimePickerDialog(context, listener, hourOfDay, minute, false) // True - 24hours format
        dialog.setCancelable(false)
        dialog.setTitle("")
        dialog.setButton(DatePickerDialog.BUTTON_POSITIVE, "SELECT", dialog)
        dialog.setButton(DatePickerDialog.BUTTON_NEGATIVE, "CANCEL", dialog)
